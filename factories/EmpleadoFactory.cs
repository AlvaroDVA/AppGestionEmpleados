...
public static class EmpleadoFactory
{
    ...
    public static Empleado GenerarEmpleadoRandom(List<Departamento> listaDepartamento)
    {
        contador += 1;
        return new Empleado(
            ...
            departamento: listaDepartamento[new Random().Next(listaDepartamento.Count)]
        );
    ...
    public static List<Empleado> GenerarListaEmpleados () {
        ...
        for ( var i = 0;i < 50;i++) {
            list.Add(GenerarEmpleadoRandom(DepartamentoFactory.GenerarDepartamentos()));
        }
        return list;
    }
}